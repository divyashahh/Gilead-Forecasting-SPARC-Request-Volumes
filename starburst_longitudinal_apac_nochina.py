...
df_china = actuals[(actuals['gilead_location_level1'] == 'APAC (Geographic)') & (actuals['gilead_location_level2'] != 'China (Geographic)')]
...
display(prep_df)
prep_df = prep_df[(prep_df.gilead_location_level1 == 'APAC (Geographic)') & (prep_df.gilead_location_level2 != 'China (Geographic)')]
display(prep_df)
...
filtered_forecast = filtered_forecast.rename(columns={'yhat':'yhat_apac_no_china'})
display(filtered_forecast)
...
merged_actuals['yhat_apac_no_china'] = merged_actuals['yhat_apac_no_china'].clip(lower=0)
display(merged_actuals)
...
plt.plot(plot_df.index, plot_df['yhat_apac_no_china'])


# In[82]:


plot_df['accuracy_by_day'] = 1-abs(plot_df['y']/plot_df['yhat_apac_no_china'])
...
merged_actuals = merged_actuals.rename(columns={'y':'y_apac_no_china'})
display(merged_actuals)
...
merged_actuals.to_csv('apac_no_china.csv', index=False, header=True)
