...
df_latam = df[df['gilead_location_level1'] == 'LATAM (Geographic)']
df_latam.head()
...
unique_countries = df_latam['gilead_location_level2'].unique()
...
df_latam = df_latam.filter(items = preserve_columns)
df_latam.dtypes
df_latam['ds'] = pd.DatetimeIndex(df_latam['ds'])
df_latam.dtypes
df_latam = df_latam.set_index(pd.DatetimeIndex(df_latam['ds'])).drop('ds', axis=1)
df_latam.plot()
...
df_latam_weekdays = df_latam.copy()
df_latam_weekdays['day_name'] = df_latam.index.day_name()
print(df_latam_weekdays.iloc[100:125])


# In[9]:


df_latam.resample('W').sum().plot()


# In[10]:


df_latam.resample('M').sum().plot()


# In[11]:


df_latam_roll = df_latam.copy()
df_latam_roll['30_day_ma'] = df_latam_roll.y.rolling(30, center=True).mean()
df_latam_roll['30_std'] = df_latam_roll.y.rolling(30, center=True).std()
df_latam_roll.plot()
...
df_latam['day'] = df_latam.index.dayofweek
df_latam['week'] = df_latam.index.strftime('%U').astype(int)
seasonal_plot(df_latam, y='y', period='week', freq='day')


# In[13]:


df_latam['dayofyear'] = df_latam.index.dayofyear
df_latam['year'] = df_latam.index.year
seasonal_plot(df_latam, y='y', period='year', freq='dayofyear')


# In[14]:


plot_periodogram(df_latam.y)
...
df_latam = actuals.copy()
df_latam = df_latam.rename(columns={'sys_created_date':'ds', 'unique_tickets':'y'})
df_latam = df_latam[(df_latam.organization_flag == 'Gilead')]
preserve_columns = ['ds', 'y']
df_latam = df_latam.filter(items = preserve_columns)
df_latam['ds'] = pd.DatetimeIndex(df_latam['ds'])
df_latam.dtypes
df_latam = df_latam.set_index('ds').to_period('D')

dp = DeterministicProcess(
    index=df_latam.index,
    constant=True,
    order=1,
    seasonal=True,
...
prep_df = prep_df[prep_df['gilead_location_level1'] == 'LATAM (Geographic)']
preserve_columns = ['ds', 'y']
