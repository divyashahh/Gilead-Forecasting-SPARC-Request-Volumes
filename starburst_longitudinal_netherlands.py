...
df_netherlands = actuals.copy()
df_netherlands = df_netherlands.rename(columns={'sys_created_date':'ds', 'unique_tickets':'y'})
df_netherlands.head()
...
df_netherlands = df_netherlands[df_netherlands['gilead_location_level2'] == 'Netherlands (Geographic)']
df_netherlands.head()
...
unique_countries = df_netherlands['gilead_location_level2'].unique()
...
df_netherlands = df_netherlands.filter(items = preserve_columns)
df_netherlands.dtypes
df_netherlands['ds'] = pd.DatetimeIndex(df_netherlands['ds'])
df_netherlands.dtypes
df_netherlands = df_netherlands.set_index(pd.DatetimeIndex(df_netherlands['ds'])).drop('ds', axis=1)
df_netherlands.plot()
...
df_netherlands_weekdays = df_netherlands.copy()
df_netherlands_weekdays['day_name'] = df_netherlands.index.day_name()
print(df_netherlands_weekdays.iloc[100:125])


# In[9]:


df_netherlands.resample('W').sum().plot()


# In[10]:


df_netherlands.resample('M').sum().plot()


# In[11]:


df_netherlands_roll = df_netherlands.copy()
df_netherlands_roll['30_day_ma'] = df_netherlands_roll.y.rolling(30, center=True).mean()
df_netherlands_roll['30_std'] = df_netherlands_roll.y.rolling(30, center=True).std()
df_netherlands_roll.plot()


# In[12]:


df_netherlands['day'] = df_netherlands.index.dayofweek
df_netherlands['week'] = df_netherlands.index.strftime('%U').astype(int)
seasonal_plot(df_netherlands, y='y', period='week', freq='day')


# In[13]:


df_netherlands['dayofyear'] = df_netherlands.index.dayofyear
df_netherlands['year'] = df_netherlands.index.year
seasonal_plot(df_netherlands, y='y', period='year', freq='dayofyear')


# In[14]:


plot_periodogram(df_netherlands.y)
...
df_netherlands = actuals.copy()
df_netherlands = df_netherlands.rename(columns={'sys_created_date':'ds', 'unique_tickets':'y'})
df_netherlands = df_netherlands[(df_netherlands.organization_flag == 'Gilead')]
preserve_columns = ['ds', 'y']
df_netherlands = df_netherlands.filter(items = preserve_columns)
df_netherlands['ds'] = pd.DatetimeIndex(df_netherlands['ds'])
df_netherlands.dtypes
df_netherlands = df_netherlands.set_index('ds').to_period('D')

dp = DeterministicProcess(
    index=df_netherlands.index,
    constant=True,
    order=1,
    seasonal=True,
...
display(prep_df)
prep_df = prep_df[prep_df['gilead_location_level2'] == 'Netherlands (Geographic)']
display(prep_df)
...
filtered_forecast = filtered_forecast.rename(columns={'yhat':'yhat_netherlands'})
display(filtered_forecast)
...
merged_actuals['yhat_netherlands'] = merged_actuals['yhat_netherlands'].clip(lower=0)
display(merged_actuals)
...
plt.plot(plot_df.index, plot_df['yhat_netherlands'])


# In[38]:


plot_df['accuracy_by_day'] = 1-abs(plot_df['y']/plot_df['yhat_netherlands'])
...
merged_actuals = merged_actuals.rename(columns={'y':'yhat_netherlands'})
display(merged_actuals)
...
merged_actuals.to_csv('netherlands.csv', index=False, header=True)
