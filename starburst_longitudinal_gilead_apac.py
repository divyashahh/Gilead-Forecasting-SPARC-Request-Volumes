...
df_apac = df[df['gilead_location_level1'] == 'APAC (Geographic)']
df_apac.head()
...
unique_countries = df_apac['gilead_location_level2'].unique()
...
df_apac = df_apac.filter(items = preserve_columns)
df_apac.dtypes
df_apac['ds'] = pd.DatetimeIndex(df_apac['ds'])
df_apac.dtypes
df_apac = df_apac.set_index(pd.DatetimeIndex(df_apac['ds'])).drop('ds', axis=1)
df_apac.plot()
...
df_apac_weekdays = df_apac.copy()
df_apac_weekdays['day_name'] = df_apac.index.day_name()
print(df_apac_weekdays.iloc[100:125])


# In[9]:


df_apac.resample('W').sum().plot()


# In[10]:


df_apac.resample('M').sum().plot()


# In[11]:


df_apac_roll = df_apac.copy()
df_apac_roll['30_day_ma'] = df_apac_roll.y.rolling(30, center=True).mean()
df_apac_roll['30_std'] = df_apac_roll.y.rolling(30, center=True).std()
df_apac_roll.plot()
...
df_apac['day'] = df_apac.index.dayofweek
df_apac['week'] = df_apac.index.strftime('%U').astype(int)
seasonal_plot(df_apac, y='y', period='week', freq='day')


# In[13]:


df_apac['dayofyear'] = df_apac.index.dayofyear
df_apac['year'] = df_apac.index.year
seasonal_plot(df_apac, y='y', period='year', freq='dayofyear')


# In[14]:


plot_periodogram(df_apac.y)
...
df_apac = actuals.copy()
df_apac = df_apac.rename(columns={'sys_created_date':'ds', 'unique_tickets':'y'})
df_apac = df_apac[(df_apac.organization_flag == 'Gilead')]
preserve_columns = ['ds', 'y']
df_apac = df_apac.filter(items = preserve_columns)
df_apac['ds'] = pd.DatetimeIndex(df_apac['ds'])
df_apac.dtypes
df_apac = df_apac.set_index('ds').to_period('D')
...
prep_df = prep_df[prep_df['gilead_location_level1'] == 'APAC (Geographic)']
preserve_columns = ['ds', 'y']
