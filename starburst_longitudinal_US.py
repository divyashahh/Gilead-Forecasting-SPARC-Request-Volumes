...
df_us = df_us[df_us['gilead_location_level2'] == 'United States of America (Geographic)']
df_us.head()
...
prep_df = prep_df[prep_df['gilead_location_level2'] == 'United States of America (Geographic)']
...
filtered_forecast = filtered_forecast.rename(columns={'yhat':'yhat_north_us'})
display(filtered_forecast)
...
merged_actuals['yhat_north_us'] = merged_actuals['yhat_north_us'].clip(lower=0)
...
plt.plot(plot_df.index, plot_df['yhat_north_us'])


# In[26]:


plot_df['accuracy_by_day'] = 1-abs(plot_df['y']/plot_df['yhat_north_us'])
...
merged_actuals.to_csv('north_us.csv', index=False, header=True)
